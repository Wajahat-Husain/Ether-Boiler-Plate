...
const CustomToken = ({ userInfo, etherObj }) => {
  ...
  const mintToken = async (e) => {
    ...
    e.preventDefault();
    let contract = new Contract(data[`0x${userInfo.connectionid}_Contract_Address`], data.ABI, etherObj)
    let decimal = await contract.decimals();
    let x = new Decimal(amount);
    let y = new Decimal(Number(`1` + "0".repeat(parseInt(decimal))));
    let a = x.mul(y);
    let quantity = new BigNumber(a);
    console.log("Big Number Price", quantity.valueOf());
    let tx = await contract.mint(address, quantity.valueOf());
    tx = await tx.wait()
    console.log(tx)
    alert(tx.hash);
    storeTransationDetails(tx.hash);  // Storing tranaction hash 
    setAddress("");
    setAmount("");
  ...
  const transferToken = async (e) => {
    ...
    let tx = await contract.transfer(address, quantity.valueOf());
    tx = await tx.wait()
    console.log(tx)
    alert(tx.hash);
    storeTransationDetails(tx.hash);  // Storing tranaction hash 
  ...
  const approveToken = async (e) => {
    ...
    let tx = await contract.approve(address, quantity.valueOf());
    tx = await tx.wait()
    console.log(tx)
    alert(tx.hash);
    storeTransationDetails(tx.hash);  // Storing tranaction hash 
  ...
  const balanceOfToken = async (e) => {
    ...
    let decimal = await contract.decimals();
    let x = new Decimal(parseInt(amount));
    let y = new Decimal(Number(`1` + "0".repeat(parseInt(decimal))));
    let a = x.div(y);
    let quantity = new BigNumber(a);
    console.log("Big Number Price", quantity.valueOf());
