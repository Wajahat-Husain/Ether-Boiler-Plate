...
const TransactionHistory = ({ userInfo, etherObj }) => {
  ...
  const fetchTransactionHistory = async (chainid) => {
    ...
    try {
      ...
      let result = await etherObj.provider.getTransactionReceipt(tokens);
      console.log(result);
      transactionHistory.push({ 'from': result.from, 'to': result.to, 'effectiveGasPrice': result.effectiveGasPrice, 'blockNumber': result.blockNumber, 'transactionHash':tokens })
    }
