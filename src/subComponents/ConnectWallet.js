...
const ConnectWallet = ({ network, userInfoValue, etherObjValue }) => {
  ...
  const onConnect = async () => {
    try {
      ...
      if (currentProvider) {
        ...
        console.log(chainId, name, networksConfig[network].chainId);
        
        if (`0x${Number(parseInt(chainId)).toString(16)}` === networksConfig[network].chainId) {
