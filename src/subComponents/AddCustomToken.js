...
const AddCustomToken = ({ userInfo , etherObj}) => {
  ...
  const tokenDetails = async (chainid) => {
   ...
   for(const tokens of data.data){
     console.log(tokens)
     let contract = new Contract(tokens, tokenAbi.ABI, etherObj)
     let tokeName = await contract.name();
     let symbol = await contract.symbol();
     let decimals = await contract.decimals();
     console.log(userInfo.account)
     let balance = await contract.balanceOf(userInfo.account);
     console.log(balance, decimals)
     balance = Number(parseInt(balance)) / Number(`1` + '0'.repeat(parseInt(decimals)))
     console.log(symbol)
     ownedTokenIds.push({'name':tokeName, 'symbol':symbol, 'decimal':parseInt(decimals), 'balance':parseInt(balance)})
   }
